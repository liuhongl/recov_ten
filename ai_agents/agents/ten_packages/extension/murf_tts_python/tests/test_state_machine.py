...
from murf_tts_python.murf_tts import (
    EVENT_TTS_RESPONSE,
    EVENT_TTS_END,
...
def test_sequential_requests_state_machine(MockMurfTTSClient):
    """
    ...
    MockMurfTTSClient.return_value = mock_instance
    ...
    class StateMachineStreamer:
        ...
        def clear_synthesizer(self, request_id: str):
            """Mock clear_synthesizer."""
            print(f"  → Mock: Clearing synthesizer for {request_id}")
    ...
    def mock_client_init(config, ten_env, vendor, response_msgs):
        ...
        mock_instance.clear_synthesizer = MagicMock(
            side_effect=streamer.clear_synthesizer
        )
    ...
    tester.set_test_mode_single("murf_tts_python", json.dumps(config))
    tester.run()
...
@patch("murf_tts_python.extension.MurfTTSClient")
def test_request_state_transitions(MockMurfTTSClient):
    """
