...
| 表名 | 作用 | 核心程度 |
|---|---|---|
| `ai_call_session` | 外呼会话主表，一次 Web 调试或真实外呼对应一条记录 | 核心 |
...
2. 当前阶段可以通过程序默认值、环境变量或接口入参完成 Agent、模型和话术配置，并把实际执行参数写入 `ai_call_session` 的配置快照字段。
3. 后续真正建设配置模块时，应根据当时的运营台、模型供应商、提示词管理、灰度发布和权限边界重新评审字段，不要求机械沿用本稿。

## 4. ai_call_session

### 4.1 表作用

`ai_call_session` 是外呼会话主表。一次外呼请求、一次 Web 调试会话或一次真实电话 Agent 通话，都对应一条 session。
...
Web 调试也创建 `ai_call_session`，但 `channel=web_debug`，SIP Call-ID 等真实线路字段为空。配置快照记录本次调试使用的 Agent、模型和话术参数。

**用例 5：业务侧直接传配置参数**

业务侧不使用模块配置表时，直接传 Agent、模型、话术参数。模块将实际执行参数保存到 `agent_config_snapshot`、`model_config_snapshot`、`script_snapshot`，便于复盘。
...
| `web_debug` | 浏览器 WebRTC 调试，不拨真实电话 |
...
2. 可来自业务侧入参，也可来自模块默认配置。
3. 常见内容包括 ASR provider、ASR model、LLM provider、LLM model、TTS provider、TTS voice、超时、温度、流式开关。
4. 密钥不写入快照，只保存 `secret_ref` 或供应商配置引用。
5. 如果参数来自 `ai_model_config`，快照中应按 ASR / LLM / TTS 分别包含 `model_config_id` 和 `config_version`。
6. 用于排查识别差、回复慢、音色不对、模型成本异常等问题。
...
`ai_call_participant` 记录一次通话中的参与者。LiveKit Room 中的 SIP 用户、Web 调试用户、Agent 和人工坐席都应有参与者记录。

### 5.2 业务场景

1. 真实电话用户作为 SIP Participant 加入 Room。
2. 浏览器调试用户作为 WebRTC Participant 加入 Room。
...
5. 调试阶段查看 ASR final 和 Agent 回复是否符合预期。
...
| `user_track` | 用户分轨，只录电话用户或 Web 调试用户；用于 ASR 重跑、用户情绪、投诉和敏感词分析 |
...
调用方可以直接传模型参数，也可以引用配置表。本表用于 Web 调试、模型切换、音色验证和生产复盘。
...
| `voice_name` | `varchar(128)` | TTS 音色，可空 |
