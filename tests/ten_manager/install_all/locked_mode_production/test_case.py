    ...
    with open(
        ...
        for ext in expected_json["extension"]:
            ...
            assert found_in_folder is True, \
                f"Extension {ext['name']} not found in {extension_folder}"
        ...
        assert len(expected_json["extension"]) == installed_extension_cnt, \
            f"Expected {len(expected_json['extension'])} extensions, but found {installed_extension_cnt}"


def test_tman_install_locked_production():
    """Test tman install --locked --production only installs dependencies, not dev_dependencies."""
    ...
    # Execute tman install --locked --production.
    ...
    if returncode != 0:
        print(output_text)
        assert False, "tman install --locked --production should succeed"

    analyze_resolve_result(app_dir)


if __name__ == "__main__":
    test_tman_install_locked_production()
