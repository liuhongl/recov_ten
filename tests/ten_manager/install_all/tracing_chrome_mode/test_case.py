...
def test_tman_tracing_chrome_mode():
    """Test that tman --tracing chrome install generates Chrome tracing JSON file."""
    ...
    # Execute tman install with --tracing chrome
    ...
    if returncode != 0:
        print(output_text)
        assert False, "tman install with --tracing chrome failed"
    ...
    with open(trace_file_path, 'r') as f:
        try:
            ...
            # Chrome tracing format should have traceEvents or be an array
    ...
    # Verify that flamegraph files are NOT created in chrome-only mode
    flame_files = [f for f in os.listdir(tracing_dir) if f.startswith("flamegraph_") and f.endswith(".svg")]
    assert len(flame_files) == 0, "Flamegraph files should not be created in chrome-only mode"

    print("✅ Chrome tracing mode test passed")


if __name__ == "__main__":
    test_tman_tracing_chrome_mode()
