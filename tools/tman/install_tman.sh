...
get_latest_version() {
    ...
    if [ -z "$latest_version" ]; then
        ...
        print_info "Using default version: 0.11.63" >&2
        latest_version="0.11.63"
    else
...
Arguments:
  VERSION    Optional. Specify a version to install (e.g., 0.11.63)
             If not provided, the latest version will be downloaded.

Examples:
  $0              # Install latest version
  $0 0.11.63      # Install specific version
